...
void C_Splash::PlaySplashSound(Vector vPos)
{
	...
	//	EmitSound( filter, SOUND_FROM_WORLD, CHAN_STREAM, "Splash.SplashSound", 1.0, ATTN_NORM, 0, 100, &vPos );
		EmitSound_t es;
		es.m_nChannel = CHAN_STREAM;
		es.m_pSoundName = "Splash.SplashSound";
		es.m_flVolume = 1.0f;
		es.m_SoundLevel = SNDLVL_NORM;
		es.m_nFlags = 0;
		es.m_nPitch = 100;
		es.m_pOrigin = &vPos;
		EmitSound(filter, SOUND_FROM_WORLD, es);
...
{
	for (Particle* pParticle = pIterator->GetFirst(); pParticle != NULL; pParticle = pIterator->GetNext())
	{
		SimpleParticle* pSimpleParticle = (SimpleParticle*)pParticle;

		//Should this particle die?
		pSimpleParticle->m_flLifetime += pIterator->GetTimeDelta();
		...
		bool viewaccess = IsCurrentViewAccessAllowed();
		AllowCurrentViewAccess(true);

		if (m_ParticleCollision.MoveParticle(pSimpleParticle->m_Pos, pSimpleParticle->m_vecVelocity, NULL, pIterator->GetTimeDelta(), &trace))
		{
		...
		AllowCurrentViewAccess(viewaccess);
	}
}

void C_Splash::RenderParticles(CParticleRenderIterator* pIterator)
{
	for (const Particle* pParticle = pIterator->GetFirst(); pParticle != NULL; )
	{
		SimpleParticle* pSimpleParticle = (SimpleParticle*)pParticle;
		...
		pParticle = pIterator->GetNext(start.z);
	}
}
